...
public record GetProductPageQueryProcessor(EntityManager entityManager) {

    public PageCarrier<ProductView> process(GetProductPage query) {
        String queryString = """
                SELECT new learn.tdd.querymodel.ProductSellerTuple(p,s) 
                FROM Product p 
                ...
                """;
        int pageSize = 10;
        List<ProductSellerTuple> resultList = entityManager.createQuery(queryString, ProductSellerTuple.class)
